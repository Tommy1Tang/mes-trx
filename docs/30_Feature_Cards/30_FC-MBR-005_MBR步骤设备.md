...
source_section: "2.5 MBR步骤设备"
requirements: ["REQ-MBR-012", "REQ-MBR-013"]
...
|--------|----------|--------|----------|----------|----------|
| REQ-MBR-012 | 步骤设备镜像 | P0++ | 接收步骤所需设备、工装、夹具、天平和传感器配置，包括是否必需、设备类型、校准要求和适用范围 | 设备定义完整 | 系统自动 |
| REQ-MBR-013 | 步骤设备校验规则镜像 | P0++ | 接收设备状态、校准有效期、点检状态等校验规则；实际可用性引用MES设备/SAP PM状态结果 | 校验规则可执行 | 系统自动 |
